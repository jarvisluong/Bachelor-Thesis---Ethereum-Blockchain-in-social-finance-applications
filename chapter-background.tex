...
A blockchain is a structure to store data in a continuously growing list of records, which are also referred to as "blocks" \citep{RefWorks:doc:WhatIsBlockChain}. These "blocks" are linked and secured by cryptography. The whole chain containing the "blocks" is protected and maintained by a network of computers, which is also called a \texttt{node network}, through a consensus mechanism.
